...
#     U_global,
#     atoms_per_env,
#     constraints_per_env,   # constraints_per_env[e][i] = list/array of constraint vectors
#     *,
#     normalize=True,
#     round_decimals=12,
# ):
#     """
#     MiniGrid-compatible SCOT greedy selection.
...
#     constraints_per_env : nested structure
#         Intended: constraints_per_env[e][i] = list[np.ndarray(d,)].
#         This implementation also tolerates:
#           - constraints_per_env[e] as (n_constraints, d) array
#           - constraints_per_env[e][i] as (d,) array
#           - accidental scalar entries (will be treated as length-1 vectors)
#     """

#     U_global = np.asarray(U_global, dtype=float)
#     if U_global.ndim != 2:
#         raise ValueError(f"U_global must be 2D, got shape={U_global.shape}")
...
#         v = np.atleast_1d(v)  # <-- critical: prevents scalar crash
...
#     # -----------------------------
#     # Make any "atom_constraints" into iterable of (d,) vectors
#     # -----------------------------
#     def as_constraint_list(atom_constraints):
...
#         # If user accidentally passed constraints_per_env[env_idx] as a (n_constraints, d) matrix,
#         # but atoms_per_env expects per-atom lists, we cannot guess which constraints belong to which atom.
#         # We will interpret each row as a standalone "atom" only if lengths match.
#         env_constraints_obj = constraints_per_env[env_idx]

#         # Normal intended case: list over atoms
#         if isinstance(env_constraints_obj, list):
#             per_atom_constraints = env_constraints_obj
#         else:
#             per_atom_constraints = env_constraints_obj  # ndarray case; handled below in loop

#         # Build per-atom coverage
#         if isinstance(per_atom_constraints, np.ndarray) and per_atom_constraints.ndim == 2:
#             # If it's (n, d) but atoms_per_env has different length, that's a structural bug upstream.
#             if len(atoms_per_env[env_idx]) not in (0, per_atom_constraints.shape[0]):
#                 raise ValueError(
#                     f"Env {env_idx}: constraints look like (n,d)={per_atom_constraints.shape} "
#                     f"but atoms_per_env has {len(atoms_per_env[env_idx])} atoms. "
#                     f"Stage-2 expects constraints_per_env[e][i]."
#                 )

#             # Interpret each row as constraints for that atom index (single constraint)
#             for i in range(per_atom_constraints.shape[0]):
#                 atom_constraints = [per_atom_constraints[i, :]]
#                 atom_cov = set()
...
#         else:
#             # Expected: iterable over atoms, each entry list/array of constraint vectors
#             for atom_constraints in per_atom_constraints:
#                 atom_cov = set()
#                 for v in as_constraint_list(atom_constraints):
#                     # sanity: allow shorter vectors, but matching keys will fail naturally
#                     k = key_for(v)
#                     if k in key_to_uix:
#                         atom_cov.update(key_to_uix[k])
#                 cov_i.append(atom_cov)
...
#         # Add the actual constraint vectors for this chosen atom (best effort)
#         chosen_atom_constraints = constraints_per_env[env_idx][atom_idx]
#         chosen_constraints_list.extend(as_constraint_list(chosen_atom_constraints))
...
#     Returns:
#         mdp_cov: list[set[int]] where each set contains universal key ids
...
#     # Per-env coverage in uid-space (atoms only, no Q leakage)
#     mdp_cov = []
#     for H_k in U_per_env_atoms:
#         cov_uids = set()
#         if H_k is None:
...
#     """
#     Greedy set cover over MDPs (unweighted).
...
# # ------------------------------------------------------------
# # Two-stage SCOT (NO-LEAK): Stage-1 uses atoms only
# # ------------------------------------------------------------
...
# ):
#     """
#     Stage-1: selects environments using ONLY atom-implied constraints (no demo/Q leakage).
#     Stage-2: runs scot_greedy_family_atoms_tracked on selected envs.
#     """

#     n_envs = len(envs)
#     if not (len(candidates_per_env) == len(SFs) == len(U_per_env_atoms) == n_envs):
#         raise ValueError("Length mismatch among envs / candidates_per_env / SFs / U_per_env_atoms")

#     U_universal = np.asarray(U_universal, dtype=float)
#     d = U_universal.shape[1]

#     # -----------------------------
#     # Stage-1: coverage in key space
#     # -----------------------------
...
#     # -----------------------------
#     # Stage-2: SCOT on restricted pool
#     # -----------------------------
...
#     # -----------------------------
#     # Stage-2: SCOT (MiniGrid-safe)
#     # -----------------------------

#     # Restrict atoms AND their constraints to selected envs
#     pool_atoms = [candidates_per_env[k] for k in selected_mdps]
#     pool_constraints = [U_per_env_atoms[k] for k in selected_mdps]

#     chosen_local, pool_stats, _ = scot_greedy_family_atoms_tracked_minigrid(
#         U_global=U_universal,
#         atoms_per_env=pool_atoms,
#         constraints_per_env=pool_constraints,
#         normalize=normalize,
#         round_decimals=round_decimals,
#     )
