...
# # Your function
# def batch_means_variance(mcmc_samples, num_batches=10):
#     m = len(mcmc_samples)
#     if m < num_batches:
#         raise ValueError("Not enough samples for batching")
#     a = m // num_batches
#     b = num_batches
#     if isinstance(mcmc_samples, list):
#         mcmc_samples = np.array(mcmc_samples)
#     batch_means = np.array([np.mean(mcmc_samples[(j-1)*a : j*a], axis=0) 
#                            for j in range(1, b+1)])
#     overall_mean = np.mean(mcmc_samples, axis=0)
#     var_estimate = (a / (b - 1)) * np.sum((batch_means - overall_mean)**2, axis=0)
#     return var_estimate
...
# Example usage
# samples = your_mcmc_samples  # Replace with your data
# entropy = kozachenko_leonenko_entropy(samples)
...
#     # Second term: -log( (1 / m_i) * sum( exp(-log P(H_{1:i} | theta^{(k)})) ) )
#     second_term = -np.log(np.mean(np.exp(-log_probs)))
    
#     # Entropy estimate
#     entropy = first_term + second_term + np.log(4)
#     return entropy
...
# def compute_log_marginal_likelihood_arithmetic(env, demos, samples, beta, log_prob_func, log_A_d):
#     """
#     Estimate log marginal likelihood using arithmetic mean importance sampling.
    
#     Args:
#         env: Environment object.
#         demos: List of demonstrations.
#         samples: MCMC samples from the posterior.
#         beta: Precision parameter for likelihood.
#         log_prob_func: Function to compute log P(H | theta).
#         log_A_d: Log surface area of the unit sphere (prior normalization).
    
#     Returns:
#         float: Log marginal likelihood estimate.
#     """
#     num_mcmc_samples = len(samples)
...
#     # Compute P(H_{1:i} | theta) = exp(log_probs)
#     likelihoods = np.exp(log_probs)
    
#     # Prior probability: P(theta) = 1 / A_d, so log P(theta) = -log A_d
#     log_prior = -log_A_d
    
#     # Importance sampling: sum P(H | theta) * P(theta) / q(theta)
#     # Since q(theta) is the posterior and samples are from q, this simplifies to mean(likelihoods) * P(theta)
#     log_marginal = np.log(np.mean(likelihoods)) + log_prior
...
#     # Marginal likelihood term: -log P(H_{1:i})
#     log_marginal = compute_log_marginal_likelihood_arithmetic(env, demos, samples, beta, log_prob_func, log_A_d)
#     marginal_term = -log_marginal
    
#     # Entropy estimate
#     entropy = first_term + prior_term + marginal_term
#     return entropy


# def sample_from_prior(d, num_samples):
#     """
#     Sample from uniform prior over unit sphere in d dimensions.
...
#     Returns:
#         np.array: Samples of shape (num_samples, d).
#     """
#     samples = np.random.normal(0, 1, (num_samples, d))
#     samples /= np.linalg.norm(samples, axis=1)[:, np.newaxis]  # Normalize to unit sphere
#     return samples
...
# # Using samples from prior step
# def compute_log_marginal_likelihood_prior(env, demos, beta, log_prob_func, num_prior_samples=10000):
#     d = len(env.feature_weights)
#     prior_samples = sample_from_prior(d, num_prior_samples)
#     log_probs = np.array([log_prob_func(env, demos, theta, beta) for theta in prior_samples])
#     max_log_prob = np.max(log_probs)
#     log_marginal = max_log_prob + np.log(np.mean(np.exp(log_probs - max_log_prob)))
#     return log_marginal
# def compute_log_marginal_likelihood_prev_posterior(env, demos, prev_samples, beta, log_prob_func):
#     """
#     Estimate log marginal likelihood using importance sampling with samples from previous posterior.
    
#     Args:
#         env: Environment object.
#         demos: List of current demonstrations (H_{1:i}).
#         prev_samples: MCMC samples from previous posterior P(w | H_{1:i-1}).
#         beta: Precision parameter for likelihood.
#         log_prob_func: Function to compute log P(H | theta).
    
#     Returns:
#         float: Log marginal likelihood estimate for P(H_{1:i}).
#     """
#     if len(prev_samples) == 0:  # Handle first demonstration (no previous samples)
#         return compute_log_marginal_likelihood_prior(env, demos, beta, log_prob_func)
    
#     num_prev_samples = len(prev_samples)
    
#     # Compute log P(H_{1:i} | theta) and log P(H_{1:i-1} | theta) for each previous sample
#     log_probs_current = np.array([log_prob_func(env, demos, theta, beta) for theta in prev_samples])
#     log_probs_prev = np.array([log_prob_func(env, demos[:-1], theta, beta) for theta in prev_samples])
    
#     # Compute ratios: P(H_{1:i} | theta) / P(H_{1:i-1} | theta)
#     log_ratios = log_probs_current - log_probs_prev
#     ratios = np.exp(log_ratios)
    
#     # Monte Carlo estimate: mean of ratios
#     log_marginal = np.log(np.mean(ratios))
    
#     return log_marginal

# def compute_entropy_importance_sampling(env, demos, samples, prev_samples, beta, log_prob_func):
#     """
#     Compute entropy using marginal likelihood estimated from previous posterior samples.
    
#     Args:
#         env: Environment object.
#         demos: List of current demonstrations (H_{1:i}).
#         samples: MCMC samples from current posterior P(w | H_{1:i}).
#         prev_samples: MCMC samples from previous posterior P(w | H_{1:i-1}).
#         beta: Precision parameter for likelihood.
#         log_prob_func: Function to compute log P(H | theta).
...
#     # Compute log P(H_{1:i} | theta) for current posterior samples
#     log_probs = np.array([log_prob_func(env, demos, theta, beta) for theta in samples])
...
#     # Marginal likelihood term: -log P(H_{1:i})
#     log_marginal = compute_log_marginal_likelihood_prev_posterior(env, demos, prev_samples, beta, log_prob_func)
#     marginal_term = -log_marginal
...
# def compute_entropy_importance_sampling(env, demos, samples, beta, log_prob_func, log_P_H):
#     num_mcmc_samples = len(samples)
...
# def compute_conditional_log_marginal(env, prev_demos, new_demos, prior_samples, beta, log_prob_func):
#     """
#     Compute the conditional log marginal likelihood log P(H_{s:t} | H_{0:s}) using importance sampling.
    
#     Args:
#         env: The environment.
#         prev_demos: Demonstrations up to the previous step.
#         new_demos: New demonstrations to add.
#         prior_samples: Samples from the prior distribution (uniform on the unit sphere).
#         beta: Rationality parameter.
#         log_prob_func: Function to compute log P(H | theta).
    
#     Returns:
#         float: Estimated log P(H_{s:t} | H_{0:s}).
#     """
#     if not new_demos:
#         return 0.0
    
#     # Combine previous and new demos for likelihood computation
#     all_demos = prev_demos + new_demos
#     log_likelihoods = np.array([log_prob_func(env, all_demos, theta, beta) for theta in prior_samples])
    
#     # Since prior_samples are drawn from P(w) = 1/A_d (uniform on the unit sphere),
#     # the importance weight P(w)/q(w) = 1, and we only need the likelihoods
#     log_P_H_st = logsumexp(log_likelihoods) - np.log(len(prior_samples))
